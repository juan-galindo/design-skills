---
name: design-prototype-to-figma
...
### Rule 1: Always use Figma catalog components — only build primitives when no match exists

**Always look up the Figma catalog first** before deciding to build anything from scratch:
- Mobile prototypes → `specs/figma-catalog/mobile-components.md`
- Web prototypes → `specs/figma-catalog/web-components.md`
- Shared assets (Icons, Flags, Currencies, Stocks, Illustrations) → `specs/figma-catalog/assets/`

The catalog is the source of truth for what exists in the DS. Match every prototype element to a
catalog entry by name, then by visual category, then by alternate names (see Phase 2). Only when
the catalog has no match — and `search_design_system` confirms it — may you fall back to
primitives.
...
When a prototype element truly has no catalog match: build it from primitives
(`figma.createFrame()`, `figma.createRectangle()`, `figma.createText()`), add a **DS Drift
annotation** explaining what was missing and which catalog names were searched, and list it in
the Phase 6 summary so the design team can close the gap.
...
**Load the Figma catalog** — before building the inventory, read the relevant catalog file so
component matching in Phase 2 has the full list of available DS components in context:
- Mobile prototype → `specs/figma-catalog/mobile-components.md`
- Web prototype → `specs/figma-catalog/web-components.md`
...
For every component in the Phase 1a inventory, determine catalog match or primitive. **Default
to a catalog match.** Primitives are the exception, not the default — every primitive must be
justified by a failed catalog + `search_design_system` lookup.

**Search strategy** (try in order before declaring "no match"):
1. **Catalog lookup** — scan the loaded `mobile-components.md` / `web-components.md` for the
   component name, then for alternate names (Alert/Banner/Toast/Notification,
   Dropdown/Select/Menu, Tag/Chip/Badge, Card variants like Card - Selection / Card - Feedback /
   Card - Product Recommendation, etc.). Catalog entries include the `pageId` — use it.
2. **Catalog by visual category** — group by what the component does: input, navigation,
   feedback, card, sheet, header. Skim the catalog table for any entry that fits.
3. **Live confirmation** — for any candidate hit, call
   `get_design_context(fileKey, pageId)` (or `get_metadata` first if the row says "no key
   nodes") to get the component key and variant props.
4. **MCP fallback** — only if the catalog turns up nothing:
   `search_design_system(fileKey, query="ComponentName", includeComponents=true)`. This catches
   components added after the catalog snapshot.
5. **Parent coverage** — check if a parent component covers it (e.g., "Card" covers
   `Card.Header`).
...
**Build the mapping table — every row needs a catalog lookup result, build approach, and drift note:**

| Code component | Catalog entry | pageId / Key | Build approach | Drift note |
|---|---|---|---|---|
| `<Button variant="primary">` | Button - PrimaryButton | `126:325` / abc123 | Import + setProperties | ✅ Clean match |
| `<CustomCard>` | — | — | Primitives | ⚠️ No catalog match — searched: Card - Selection, Card - Feedback, Card - Product Recommendation; confirmed via `search_design_system` |
| `<DataTable sortable>` | Table | `…` / def456 | Import + setProperties | ⚠️ No `sortable` variant in Figma |

Any row with ⚠️ must get a DS Drift annotation in Phase 4. A "—" in the Catalog entry column
requires evidence that both the catalog and `search_design_system` were checked.
...
Frame size: match the CSS-measured viewport — **mobile frames are always 375px wide** (e.g.,
375×812), desktop ~1440×900. For scrollable states, set frame height to full content height —
never clip at viewport height. Add a fold marker line at the viewport height. ~200px gaps
between frames, ~400px between sections.
