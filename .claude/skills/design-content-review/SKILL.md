...
1. `.claude/skills/design-content-write/references/criteria.md` — Criterios de evaluación por tipo de contenido.
2. `.claude/skills/design-content-write/references/feel-framework.md` — Decision flow para identificar el tono FEEL correcto.
3. `.claude/skills/design-content-write/references/glossary.md` — Términos prohibidos, aprobados y reglas de localización.
4. `.claude/skills/design-content-write/references/cta-guidelines.md` — Reglas para CTAs y botones.
5. `.claude/skills/design-content-write/references/bitso-tone-of-voice.md` — Los tres pilares de voz de Bitso, ejemplos de microcopy por pilar y patrones a marcar en revisión.
6. `.claude/skills/design-content-write/references/ux-writing-principles.md` — Principios de claridad, concisión, consistencia, errores, empty states, CTAs, onboarding y notificaciones en es_MX fintech.
7. `.claude/skills/design-content-write/references/system-thinking.md` — Framework de pensamiento sistémico: cadenas de contenido, arcos emocionales y heurísticas.

**Solo si el contenido es legal** (inversiones, crypto, SPEI, rendimientos, KYC), leer también:
- `.claude/skills/design-content-review/references/compliance-mx.md` — Términos prohibidos, disclaimers requeridos, terminología correcta y checklist pre-publicación para fintech/crypto en México.

**Solo si el usuario especifica que está revisando un email o push notification**, leer también:
- `.claude/skills/design-content-write/references/transactional-emails.md` — Criterios específicos para emails transaccionales y push notifications.
...
Apply criteria from `.claude/skills/design-content-write/references/criteria.md` for the content type identified above.

**For any Bitso content** (in-app UI, emails, push notifications, Figma screens), also do:

1. **Tone check** — Use the decision flow in `.claude/skills/design-content-write/references/feel-framework.md` to identify the correct FEEL tone for the context, then assess whether the content actually uses it.
2. **Brand voice check** — Identify which pillar(s) from `.claude/skills/design-content-write/references/bitso-tone-of-voice.md` apply to the content type and context (dial up/down guidance), then assess whether the copy reflects the correct pillar and doesn't fall into the listed common mistakes. Use the **Issues to Flag in Review** table for pre-classified severity patterns.
3. **Glossary check** — Flag any violation from `.claude/skills/design-content-write/references/glossary.md`:
...
4. **UX writing principles check** — Apply rules from `.claude/skills/design-content-write/references/ux-writing-principles.md` for the content type:
...
5. **Systemic thinking check** — Apply the 5 context questions from `.claude/skills/design-content-write/references/system-thinking.md` to UI copy and flows:
...
6. **Compliance check (es_MX fintech/crypto)** *(solo si el contenido incluye inversiones, crypto, SPEI, rendimientos o KYC)* — Run the pre-publication checklist from `.claude/skills/design-content-review/references/compliance-mx.md`:
...
> For **Bitso in-app UI copy and flows**, replace the scores above with the FEEL checklist from `.claude/skills/design-content-write/references/feel-framework.md`.
> For **Bitso transactional emails and push notifications**, replace the scores above with the FEEL checklist from `.claude/skills/design-content-write/references/transactional-emails.md`.
