...
3. Apply:
   - Variable: copy `node.fills`, call `figma.variables.setBoundVariableForPaint(fillsCopy[i], 'color', variable)` for each solid paint, assign the returned `SolidPaint` back into the copy, then reassign `node.fills = fillsCopy`
...
- Variable: copy `node.strokes`, call `figma.variables.setBoundVariableForPaint(strokesCopy[i], 'color', variable)` for each solid paint, assign the returned `SolidPaint` back into the copy, then reassign `node.strokes = strokesCopy`
