...
Read `specs/patterns/scope/bullet-points.md` to get pattern rules before proceeding.
```
...
- **Pattern** — copy `specs/_template-pattern.md` to `specs/patterns/{scope}/{id}.md` where `{scope}` is `scope/` (slot-level) or `layouts/` (full-screen). Set `category: pattern`. See `specs/patterns/CLAUDE.md` and pilot `specs/patterns/scope/bullet-points.md`.
